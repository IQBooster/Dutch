...
- I have a plan.                                                                                                                                                              
	...
	{
		if (std::strcmp(label, "HUD_JOINING") == 0) return "Loading GTA Online with Dutch";
		if (std::strcmp(label, "HUD_TRANSP") == 0) return "I have a plan!";
		if (std::strcmp(label, "PM_EXIT_GAME") == 0) return "Exit GTAV with ThunderX";
		if (std::strcmp(label, "PM_ENTER_MP") == 0) return "Play GTA Online with Dutch!";
		if (std::strcmp(label, "PM_INF_PGOB") == 0) return "Play GTA Online with ThunderX. Dutch is currently undetected so no need to worry, but please do not abuse money methods!";
	...
	*/
	...
	void Hooking::FindPatterns()
	{
		...
		Log::Msg("Did you know? Dutch said word money in RDR2 more than 100x times!");
		Log::Msg("Anyway lets get back to injecting.");
		Log::Msg(Mod_Menu_Name " has been succesfully loaded! There you go injected really fast as you can see. Open with F5.");
		//Log::Msg("Join our discord server: https://tabmax.lol/dc.html");
		//Log::Msg("Our official website is: https://tabmax.lol/");
		//system("start https://discord.gg/2NysKzvwg7");
